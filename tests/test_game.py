...
@pytest.mark.parametrize("bet, dealer_hand, payout", [
    ...
    (Bet(PlayerHand([Card(7,'S'), Card('J','S')]), state=HandState.DOUBLE), DealerHand([Card('K','H'), Card(6,'D'), Card(7,'S')]), 4), # dealter bust + double
    (Bet(PlayerHand([Card(9,'S'), Card('J','S')])), DealerHand([Card('A','H'), Card(7,'D')]), 2), # player total >
    (Bet(PlayerHand([Card(9,'S'), Card('J','S')]), state=HandState.DOUBLE), DealerHand([Card('A','H'), Card(7,'D')]), 4), # player total > + double
